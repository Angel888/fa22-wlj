...
//                Tile aa =_board.tile(col,row);
                Tile aaDown =_board.tile(col,row+1);
                //todo 怎样判断aa是不是合并得到的？需不需要我自己维护一个移动的状态
                if (aaDown!=null){
                        _board.move(row,col,aaDown);
                    }
                }
            }
        checkGameOver();
    ...
    public static boolean maxTileExists(Board b) {
        ...
        for (int col = 0; col < size; col += 1) {
            for (int row = 0; row < size; row += 1) {
                Tile aa = b.tile(col, row);
                if (! Objects.isNull(aa) && aa.value()==MAX_PIECE) {
                    return true;
                }
            }
        }
    ...
    public static boolean hasSameAdjacentTiles(Board b){
        // https://blog.csdn.net/m0_51530227/article/details/123777296
    ...
    public static boolean atLeastOneMoveExists(Board b) {
        // TODO: Fill in this function.
        // 先判断有没有空的位置；没空位置的话， 横着看有米有一样的 竖着看有咩有一样的
        boolean b1 = true;
        if (emptySpaceExists(b))
            return b1;
        else{ // condition 2
            // ToDo 不需要考虑一样的棋子是不是上次动过的吗？
